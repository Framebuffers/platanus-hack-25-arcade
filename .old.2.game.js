...
const config = {
  ...
  // Minimal vector font - each array is ONE continuous stroke [x1,y1, x2,y2, x3,y3, ...]
  // Coordinates: 0=top/left edge, 0.5=center, 1=bottom/right edge
  const VECTOR_FONT = {
    'v': [[0,0, 0.5,1, 1,0]], // valley
    'i': [[0.5,0, 0.5,1]], // vertical line
    'b': [[0,0, 0,1, 0.7,1, 0.7,0.5, 0,0.5, 0.7,0.5, 0.7,0, 0,0]], // continuous path
    'e': [[1,0, 0,0, 0,0.5, 0.6,0.5], [0,0.5, 0,1, 1,1]], // two strokes
    'a': [[0,1, 0.5,0, 1,1], [0.25,0.5, 0.75,0.5]], // ^ shape + crossbar
    't': [[0,0, 1,0], [0.5,0, 0.5,1]], // two strokes
    'r': [[0,1, 0,0, 0.7,0, 0.7,0.5, 0,0.5], [0.7,0.5, 1,1]], // P + leg
    '0': [[0,0, 1,0, 1,1, 0,1, 0,0], [0,0, 1,1]], // O with slash
    '1': [[0.3,0.2, 0.5,0, 0.5,1]], // leaning line
    '2': [[0,0, 1,0, 1,0.5, 0,1, 1,1]], // step down
    '3': [[0,0, 1,0, 1,1, 0,1], [1,0.5, 0.5,0.5]], // two bumps
    '4': [[0,0, 0,0.6, 1,0.6], [0.7,0, 0.7,1]], // open 4
    '5': [[1,0, 0,0, 0,0.5, 1,0.5, 1,1, 0,1]], // S mirrored
    '6': [[1,0, 0,0.5, 0,1, 1,1, 1,0.5, 0,0.5]], // loop bottom
    '7': [[0,0, 1,0, 0.5,1]], // angle down
    '8': [[0,0, 1,0, 1,0.5, 0,0.5, 0,0], [0,0.5, 1,0.5, 1,1, 0,1, 0,0.5]], // two loops
    '9': [[1,1, 1,0, 0,0, 0,0.5, 1,0.5]], // loop top
    ' ': [] // empty
  };
  ...
  function drawTitleScreen() {
    gfx.clear();
    drawCenteredVectorText(GAME_NAME, 50, 80);
  }
  ...
  function drawVectorChar(x, y, w, h, strokes) {
    ...
    strokes.forEach(stroke => {
      if (stroke.length < 2) return; // skip empty strokes
  ...
  function drawVectorText(text, x, y, charWidth, charHeight) {
    ...
    chars.forEach((char, idx) => {
      ...
      drawVectorChar(x + idx * spacing, y, charWidth, charHeight, strokes);
    });
  }
  
  function drawCenteredVectorText(text, charWidth, charHeight) {
    const totalWidth = text.length * charWidth * 1.2;
    const x = (config.width - totalWidth) / 2;
    const y = (config.height - charHeight) / 2;
    
    drawVectorText(text, x, y, charWidth, charHeight);
  }
